...
def updateSB():
    ...
    addStatusText("Scoreboard updated")
...
def show(b):
    match b.config("text")[-1]:
        case "Load to OBS":
            b.grid(row=1, column=1, columnspan=2, padx=10, pady=7)
        case "Clear":
            b.grid(row=1, column=4, columnspan=2, padx=10, pady=7)
...
if not configs.getboolean("configurations", "autoclear"):
    clearPartsButton.grid(row=1, column=4, columnspan=2, padx=10, pady=7)
...
p2name.grid(row=4, column=4, columnspan=3, padx=7, pady=(0, 7))
...
switch.grid(row=4, column=3, rowspan=2)

updateSbButton = ttk.Button(mainTab, text="Update SB", command=updateSB)
updateSbButton.grid(row=6, column=2, columnspan=3, padx=10, pady=7)

## status queue
statusQueue = ttk.Label(mainTab, justify="center")
statusQueue.grid(row=7, column=0, columnspan=7)

### config
ceLabel = ttk.Label(configTab, text="Clear Text  ")
ceLabel.grid(row=1, column=0, pady=7, sticky='e')
clearEntry = tk.Text(configTab, width=14, height=3, font=("Arial", 9))
clearEntry.grid(row=1, column=1, pady=7, sticky='w')
clearEntry.insert(1.0, configs.get("configurations", "cleartext"))

saveClearText = ttk.Button(configTab, text="Save Clear Text", command=updateClearText)
saveClearText.grid(row=2, column=0, columnspan=2, pady=7)
...
loadTickBox.grid(row=1, column=2, padx=10, pady=7)

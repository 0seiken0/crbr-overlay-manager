...
This is a tool for managing overlays while streaming/recording Custom Robo: Battle Revolution being played through Dolphin. I designed this for OBS but it works fine with any recording software that allows text sources to read from a text file. The portion of this tool for loading parts to the screen works with both Vanilla CRBR & *Project Hive 3.1*, a patch made by the Custom Robo community and used for the community's tournaments. This will be updated in future if new parts are added or parts are reordered.
...
If you'd rather follow a video, click [here](https://www.youtube.com/watch?v=UlpfOva9VVs). The core set up inormation is still correct, but I've made several changes since making that video so many things may look different.

1. Download the most recent `crbr-overlay-manager.zip` file in releases, then extract the file. I recommend you put the extracted folder somewhere you won't need to move it, as you'll need to reconfigure the text sources you make in the next step if the folder's location changes.
...
<img width="306" height="246" alt="image" src="https://github.com/user-attachments/assets/20a6711d-a2cd-409d-a773-4f3326817bc6" />

4. To load parts, run CRBR through Dolphin and navigate to the P1+P1vsP2+P2 tag parts select screen. Click the `Load Parts` button to populate each robo's current parts into the corresponding text sources. Click the `Clear Parts` button to clear to those text sources if players are still picking builds.
...
<img width="306" height="246" alt="image" src="https://github.com/user-attachments/assets/cb2a9b3c-e24e-4f22-b428-a1addcbf5a96" />
